...
def ProcessScan():
    ...
    for proc in psutil.process_iter():     #iterate running process
        info = proc.as_dict(attrs=["pid","name","username","status"])
        info["cpu_percent"] = proc.cpu_percent(None) #key add
...
def platformServillence(FolderName):   #Create folder
  ...
  #########add time#######################
    ...
    fobj.write("RAM Usage : %s %%\n" %memory.percent)   #charastarystic of the class
    fobj.write("TOatal RAM Availbale : %s\n" %memory.total)
    fobj.write(Border+"\n")
    
    #########################Network Usage(if system conect in internet) ##############################
    Netobj = psutil.net_io_counters()
    fobj.write("Network Usage Report")
    fobj.write("Send : %.2f MB \n "%(Netobj.bytes_sent / (1024 * 1024))) #mb calculate
    fobj.write("Receive : %.2f MB \n "%(Netobj.bytes_recv / (1024 * 1024)))  #mb calculate
    fobj.write(Border+"\n")
    
    
    ##############Process Log#######################33333333333
    Data = ProcessScan()
    for info in Data:
        #fobj.write(f"{info} \n") 
        fobj.write("PID : %s \n " %info.get("pid"))
        fobj.write("Name : %s \n " %info.get("name"))
        fobj.write("User_Name : %s \n " %info.get("username"))
        fobj.write("Status : %s \n " %info.get("status"))
        fobj.write("Usage CPU : %.2f \n " %info.get("cpu_percent"))
